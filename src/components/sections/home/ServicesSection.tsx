...
export const ServicesSection = () => {
  ...
  return (
    <section className="relative bg-background py-20 sm:py-24 lg:py-8" aria-labelledby="services-heading">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <AnimateIn direction="up" className="mx-auto max-w-2xl text-center">
          ...
          <h2 id="services-heading" className="mt-4 text-3xl font-extrabold tracking-tight text-white sm:text-4xl">
            {t("services.heading")}
          </h2>
          ...
          {items.map((service, index) => {
            ...
            return (
              <StaggerItem key={index}>
                ...
                >
                  <div className={`absolute inset-x-0 top-0 h-[2px] bg-gradient-to-r opacity-0 transition-opacity duration-400 group-hover:opacity-100 ${colors.topBar}`} aria-hidden />
                  <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_left,rgba(56,152,212,0.06)_0%,transparent_70%)] opacity-0 transition-opacity duration-500 group-hover:opacity-100" aria-hidden />

                  <div className="relative flex h-full flex-col p-8">
                    <span className={`absolute right-6 top-5 select-none font-extrabold text-[4rem] leading-none tracking-tight pointer-events-none transition-all duration-300 group-hover:opacity-100 opacity-[0.06] ${colors.num}`} aria-hidden>
                      {String(index + 1).padStart(2, "0")}
                    </span>

                    <div className={`relative mb-6 inline-flex size-14 items-center justify-center rounded-xl ring-1 transition-all duration-300 group-hover:scale-110 group-hover:shadow-[0_0_20px_rgba(56,152,212,0.3)] ${colors.icon}`} aria-hidden>
