...
const CONTACT_PAGE_CONTENT = {
  location: {
    mapsQuery: "TCC+Tower+One+Menara+Batavia+Jl.+K.H.+Mas+Mansyur+Karet+Tengsin+Jakarta+Pusat"
  }
};

const mapsEmbedSrc = `https://maps.google.com/maps?q=${CONTACT_PAGE_CONTENT.location.mapsQuery}&t=&z=15&ie=UTF8&iwloc=&output=embed`;
const mapsDirectionsHref = `https://maps.google.com/maps?daddr=${CONTACT_PAGE_CONTENT.location.mapsQuery}`;

export const ContactLocationSection = () => {
  const { t } = useTranslation("contact");
  const locationItems = (t("location.items", { returnObjects: true }) || []) as Array<{ id: string; label: string; value: string }>;

  return (
    <section className="bg-background py-20 border-y border-white/5 sm:py-24 lg:py-8" aria-labelledby="contact-location-heading">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <AnimateIn direction="up" className="mb-12">
          ...
          <h2 id="contact-location-heading" className="mt-4 text-3xl font-extrabold tracking-tight text-white sm:text-4xl">
            {t("location.heading")}
          </h2>
        </AnimateIn>
          ...
          <AnimateIn direction="right">
            <div className="rounded-3xl border border-white/10 bg-surface/40 backdrop-blur-md p-6 shadow-xl sm:p-8">
              <StaggerChildren staggerDelay={0.1} className="space-y-5">
                {locationItems.map(({ id, label, value }, idx) => {
                  let Icon = MapPin;
                  let iconBg = "bg-accent/10 text-accent ring-accent/20";
                  let valueClass = "text-sm leading-relaxed text-white/75 sm:text-base";
