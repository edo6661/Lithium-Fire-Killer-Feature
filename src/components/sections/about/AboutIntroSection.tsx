...
const HIGHLIGHT_ICONS = [ShieldCheck, Award, Users];

export const AboutIntroSection = () => {
  const { t } = useTranslation("about");

  const paragraphs = t("intro.paragraphs", { returnObjects: true }) as string[];
  const highlights = t("intro.highlights", { returnObjects: true }) as string[];
  const coreValueItems = t("intro.coreValues.items", { returnObjects: true }) as Array<{ letter: string; label: string }>;
    ...
    >
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid items-start gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:gap-16">
          ...
          >
            <h1 id="about-intro-heading" className="text-3xl font-extrabold leading-[1.12] tracking-tight sm:text-4xl lg:text-5xl">
              {t("intro.heading")}
            </h1>
            <div className="mt-8 space-y-5">
              {paragraphs.map((paragraph, idx) => (
                <p key={idx} className="text-base leading-relaxed text-foreground-muted sm:text-lg">
                  {paragraph}
                </p>
              ))}
            </div>
          ...
          <StaggerChildren className="flex flex-col gap-3.5 lg:sticky lg:top-24" staggerDelay={0.15}>
            {highlights.map((label, idx) => {
              const Icon = HIGHLIGHT_ICONS[idx] || ShieldCheck;
              return (
                <StaggerItem key={idx}>
        ...
        >
          <p id="about-core-values-heading" className="text-xs font-bold uppercase tracking-widest text-accent">
            {t("intro.coreValues.title")}
          </p>
          <p className="mt-2 font-extrabold tracking-[0.35em] text-white sm:text-lg" aria-label={t("intro.coreValues.acronym")}>
            {t("intro.coreValues.acronym")}
          </p>
          <ul className="mt-5 grid gap-2.5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
            {coreValueItems.map(({ letter, label }, idx) => (
              <li key={idx} className="flex items-center gap-3 rounded-xl border border-white/5 bg-background/40 px-4 py-3">
                <span className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-accent/15 text-sm font-extrabold text-accent ring-1 ring-accent/25" aria-hidden>
                  {letter}
                </span>
