...
export const EpcMethodBlock = ({ epc }: { epc: any }) => {
  return (
    <div className="mt-10 border-t border-white/8 pt-10" aria-labelledby="epc-heading">
      <div className="grid gap-10 lg:grid-cols-[1fr_400px] lg:items-start lg:gap-12">
        <article className="space-y-5">
          ...
          <h4 id="epc-heading" className="text-2xl font-extrabold leading-[1.12] tracking-tight text-white sm:text-3xl">
            {epc.heading}
          </h4>
          ...
          <StaggerChildren staggerDelay={0.1} className="flex flex-wrap gap-3 pt-1">
            {EPC_LETTERS.map(({ letter, word, color }, idx) => (
              <StaggerItem key={idx}>
                <motion.div
                ...
                >
                  ...
                  <div>
                    <p className="text-[10px] font-extrabold uppercase tracking-widest opacity-55">{epc.phaseLabel}</p>
        ...
        <div className="rounded-2xl border border-white/10 bg-background/50 p-6 sm:p-7">
          ...
          <StaggerChildren className="list-none space-y-3" staggerDelay={0.1}>
            {epc.steps.map((step: string, idx: number) => {
              const sc = STEP_COLORS[idx] ?? STEP_COLORS[0];
              return (
                <StaggerItem key={idx}>
                  <li className="group flex items-center gap-4">
                    <div className={`flex size-11 shrink-0 items-center justify-center rounded-xl border text-sm font-extrabold shadow-sm transition-all duration-300 group-hover:scale-110 ${sc.num}`}>
                      {idx + 1}
                    </div>
